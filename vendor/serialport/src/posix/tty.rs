...
impl TTYPort {
    ...
    pub fn send_break(&self, duration: BreakDuration) -> Result<()> {
        match duration {
            BreakDuration::Short => nix::sys::termios::tcsendbreak(self.as_fd(), 0),
            BreakDuration::Arbitrary(n) => nix::sys::termios::tcsendbreak(self.as_fd(), n.get()),
        }
        .map_err(|e| e.into())
    }
...
impl io::Write for TTYPort {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        nix::unistd::write(self.as_fd(), buf).map_err(|e| io::Error::from(Error::from(e)))
    }

    fn flush(&mut self) -> io::Result<()> {
        let timeout = Instant::now() + self.timeout;
        loop {
            return match nix::sys::termios::tcdrain(self.as_fd()) {
                Ok(_) => Ok(()),
                Err(Errno::EINTR) => {
